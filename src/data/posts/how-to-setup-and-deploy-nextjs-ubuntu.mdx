...
<CommandLine>`curl -sL https://deb.nodesource.com/setup_18.x -o nodesource_setup.sh`</CommandLine>

Run the Nodesource setup file to prepare the correct version of Node.js

<CommandLine>sudo bash nodesource_setup.sh</CommandLine>

Install Node.js from the installed Nodesource

<CommandLine>sudo apt install nodejs -y</CommandLine>

Verify installation:

<CommandLine>node -v # 18.x.x</CommandLine>

#### NGINX

NGINX will be used as a proxy to serve our Next.js project.

<CommandLine>sudo apt install nginx</CommandLine>
